<?php
/**
 * Post AJAX mobile partial.
 *
 * This partial returns data used to render mobile data.
...
if ( ! $mobile ) {
	$mobile = SEOToolSetAPI::api_request( 'GET', "/posts/{$post_id}/mobile" );
}

if ( isset( $mobile['code'] ) ) {
	$alert = __( $mobile['message'], SEOTOOLSET_TEXTDOMAIN );
...
if ( ! function_exists( 'priority_fixes_level_html' ) ) {
	 ...
	 *
	 * @param array  $fixes Array of fixes by priority.
...
HTML;
									...
									echo <<<HTML
  <input id="collapsible_mobile_{$i}" class="toggle" type="checkbox">
  <label for="collapsible_mobile_{$i}" class="lbl-toggle">{$blocks['header']}</label>
...
if ( ! function_exists( 'priority_fixes_html' ) ) {
	 ...
	 *
	 * @param array  $data Array of priority issues.
...
priority_fixes_html( $mobile, __( 'mobile friendliness', SEOTOOLSET_TEXTDOMAIN ) );
?>
	<script>
